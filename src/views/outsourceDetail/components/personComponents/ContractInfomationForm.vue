    ...
    <div>
      <a-form :model="contractInfomatiomForm" :label-col="labelCol"  @finish="handleSubmit">
        <a-row :gutter="24">
          ...
          <a-row :gutter="24">
            <a-col :span="12" v-for="item in componentsShow" :key="item.componentId">
...
const components = computed(() => getEsignTemplateDetail.value?.participants?.filter(item => item.participantFlag === '签署方1')[0]?.components.map(item => {
...
const handleSubmit = () => {
  ...
  const submitData = {
    ...
    requestParam: {
      participants: [
      {
        participantId: getEsignTemplateDetail.value?.participants?.filter(item => item.participantFlag === '签署方1')[0]?.participantId || '',
        orgParticipant: {
      ...
      {
        participantId: getEsignTemplateDetail.value?.participants?.filter(item => item.participantFlag === '签署方2')[0]?.participantId || '',      
        psnParticipant: {
          psnAccount: componentsReal().find(item => item.componentName === '联系电话')?.componentValue || '',
    ...
    signFlowConfig: {
    signFlowTitle: getEsignTemplateList.value?.find(item => item.value === contractInfomatiomForm.value.signTemplate)?.label || '',
    autoFinish: true,
    autoStart: true,
    signConfig: {
