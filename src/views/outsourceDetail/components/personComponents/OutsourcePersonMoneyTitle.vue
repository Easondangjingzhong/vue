...
const defaultColumnsSalaryCompanyJian = [
  ...
  const defaultColumnsSalaryCompanyQuan = [
    {
      rowName: '序号',
      rowOther: '序号',
      show: true,
...
const configColumnsSalaryCompanyJian = ["之禾","林弥珥","艾秘","蒂芙尼","轩尼诗"];
const configColumnsSalaryCompanyQuan = ["古奇拉利","古驰","杰尼亚","瑭姆步朗","盟可睐","艾派品"];
watch(outsourcePersonMoneyTitleFlag, async (val) => {
  if (val && formStatePersonMoney.value.companyName) {
    ...
    if (savedColumns && savedColumns.length > 0) {
      ...
      if (configColumnsSalaryCompanyJian.includes(formStatePersonMoney.value.companyName)) {
        configColumnsSalary.value = defaultColumnsSalaryCompanyJian.map(col => ({
          ...col,
          rowName: col.rowName,
          rowOther: col.rowOther,
          show: col.show,
      ...
      } else if (configColumnsSalaryCompanyQuan.includes(formStatePersonMoney.value.companyName)) {
        configColumnsSalary.value = defaultColumnsSalaryCompanyQuan.map(col => ({
          ...col,
          rowName: col.rowName,
          rowOther: col.rowOther,
          show: col.show,
