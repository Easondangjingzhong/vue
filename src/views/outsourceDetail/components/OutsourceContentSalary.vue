      ...
      <a-row :gutter="24">
        ...
        <a-col :span="3">
          <a-form-item name="jobType" label="性质">
            <a-select
              optionFilterProp="label"
              v-model:value="formStateSalary.jobType"
              :showArrow="false"
              showSearch
              allowClear
            >
  ...
  <div class="resume-content">
    <a-row style="justify-content: space-between;margin-bottom: 5px;">
        <span>
          ...
          <a-button @click="handleSearchOutsourcePerson('4')" :class="{'active': formStateSalary.companyArrange === '1'}" style="margin-right: 5px;" size="small" title="按公司排序">排序</a-button>
        </span>
    </a-row>
    <a-row>
    <a-table
...
const columnsOutsourceDetail:TableColumnsType = [
  ...
  { title: '城市', dataIndex: 'city', key: 'city', fixed: 'left', width: 40, ellipsis: true },
  { title: '性质', dataIndex: 'jobType', key: 'jobType', fixed: 'left', width: 30, ellipsis: true },
  { title: '状态', dataIndex: 'currentStatus', key: 'currentStatus', fixed: 'left', width: 30, ellipsis: true },
  { title: '周期', dataIndex: 'changeTime', key: 'changeTime', fixed: 'left', width: 60, ellipsis: true },
  ...
  { title: '一金标准', dataIndex: 'yijinStandard', key: 'yijinStandard', width: 50, },
...
const clearFromState = () => {
 ...
 const handleSearchOutsourcePerson = (status) => {
     if (status != '4') {
      ...
      formStateSalary.value.companyArrange = '1';
    }
    onSearch();
  }
const onSearch = () => {
