...
const loginVueUser: { loginName: ''; loginId: ''; loginTocken: ''; loginType: '';loginOutFlag: ''; } = JSON.parse(
  ...
  actions: {
    ...
    async fetchInfo(params: PlagiarusnItem) {
      ...
      if (loginVueUser?.loginOutFlag == "1") {
        res = await fetchApi.infoWai(params);
      } else {
        res = await fetchApi.info(params);
      }
      if (res) {
        const { code, info } = res;
        let resulet = {} as ResInfoList;
        resulet.code = code;
        if (code !== 1 && Array.isArray(info) && info.length > 0) {
          resulet.info = info.map((item, index) => {
            ...
            item.birthYear =
              item.birthYear <= 0 ? 0 : parseInt((yearNow - +item.birthYear).toString());
