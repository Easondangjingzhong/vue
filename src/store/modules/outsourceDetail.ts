  ...
  getters: {
    ...
    getOutsourceMonthSalaryOfferList: (state) =>
      state.outsourceMonthSalaryOfferList.map((item, index) => ({
        ...
        userNameCn: item.userNameCn
          ...
          : '',
        costTotalke: (
          Number(item.companyShebaoKe || 0) +
          Number(item.companyYijinKe || 0) +
        ...
        costTotal: (
          Number(item.companyShebao || 0) +
          Number(item.companyYijin || 0) +
        ...
        totalChargeAfter: item.totalCharge
          ? (parseFloat(item.totalCharge) / 1.0672).toFixed(2)
          : '',
      })),
    ...
    getOutsourceFormulaList: (state) =>
      state.outsourceFormulaList.map((item, index) => ({
        ...item,
        index:
          state.pageOutsourceFormulaList.pageSize *
    ...
    getOfferOutsourceMonthSalary: (state) => {
      const formulaList = state.outsourceFormulaList[0];
      return state.offerOutsourceMonthSalary.map((item, index) => {
        ...
        if (monthStr) {
          ...
          if (formulaList.cycle === '1') {
            ...
            const cycleLabel =
              cycleFormulaOption.find((item) => item.value === formulaList.cycle)?.label || '';
            const parsePart = (part: string) => {
              const match = part.match(/^(上月|当月|下月)(\d{1,2})日$/);
              if (!match) {
                return null;
              }
              const scope = match[1];
              ...
              const monthOffset = scope === '上月' ? -1 : scope === '下月' ? 1 : 0;
              return { dayNum, monthOffset };
            };
            const formatYM = (baseMonthStr: string, monthOffset: number, dayNum: number) => {
              const base = dayjs(`${baseMonthStr}-01`).add(monthOffset, 'month');
              const ym = base.format('YYYY/MM');
              return `${ym}/${String(dayNum).padStart(2, '0')}`;
            };
            ...
            if (!cycleLabel || !cycleLabel.includes('-')) {
              jinxinMonth = computeDefaultRange();
            } else {
              const [leftRaw, rightRaw] = cycleLabel.split('-');
              const left = parsePart(leftRaw.trim());
              const right = parsePart(rightRaw.trim());
              if (!left || !right) {
                jinxinMonth = computeDefaultRange();
              } else {
                const startStr = formatYM(monthStr, left.monthOffset, left.dayNum);
                const endStr = formatYM(monthStr, right.monthOffset, right.dayNum);
                jinxinMonth = `${startStr}-${endStr}`;
              }
            }
          }
        }
        ...
        const parseRateFromManageGongShi = (val: unknown) => {
          ...
          const starMatch = str.match(/\*\s*(\d+(?:\.\d+)?)/);
          if (starMatch) {
        ...
        const allHours = (
          ...
          parseFloat(item.daixinBingjiaHours || '0') -
          parseFloat(item.kouxinBingjiaHours || '0') +
          parseFloat(item.otherDaixinHours || '0') -
          parseFloat(item.shijiaHours || '0') +
          parseFloat(item.nianjianHours || '0') +
          parseFloat(item.hunjiaHours || '0') +
        ...
        return {
          ...
          mkName: item.mkName || item.market,
    ...
    getOutsourcePersonPerformanceDetailGongShi: (state) =>
      state.outsourcePersonPerformanceDetailGongShi,
    ...
    getOutsourcePersonPerformanceDetailCostInfo: (state) => {
      const costArr = [] as CostItem[];
      const sheBaoMap =
        state.outsourcePersonPerformanceDetailSheBaoInfo.length > 0
      ...
      costArr.push({
        ...
        manageChargeRate: costMap.manageChargeRate,
        manageChargeTaxMoney: costMap.manageChargeTaxMoney,
        moneyCahrgeTax: costMap.moneyCahrgeTax,
        zhuanChargeTax: costMap.zhuanChargeTax,
        zhuanChargeRate: costMap.zhuanChargeRate,
        zhuanChargeTaxMoney: costMap.zhuanChargeTaxMoney,
        zhuanChargeAfter: costMap.zhuanChargeAfter,
        totalCharge: costMap.totalCharge,
        manageChargeAllocationAfter: costMap.manageChargeAllocationAfter,
      });
      costArr.push({
        costType: '公司账单',
        monthTax: costMap.monthTax,
    ...
    getOutsourcePersonSalaryCommitCollectSheBao: (state) =>
      state.outsourcePersonSalaryCommitCollectSheBao.map((item, index) => ({
        ...item,
        index: index + 1,
        userName: `${item?.userNameCn || ''}${item?.userNameEn ? '/' + item?.userNameEn : ''}`,
        companyTotal: parseFloat((item?.companyTotal || 0).toFixed(2)),
        personTotal: parseFloat((item?.personTotal || 0).toFixed(2)),
        shebaoTotal: parseFloat(
          (
  ...
  actions: {
    ...
    async queryOutsourceSheBao() {
      ...
      if (!this.formStateSheBao.yearAndMonth) {
        const now = dayjs();
        this.formStateSheBao.yearAndMonth =
          now.date() <= 15 ? now.subtract(1, 'month').format('YYYY-MM') : now.format('YYYY-MM');
      }
    ...
    async queryOutsourceMonthSalary() {
      ...
      if (!this.formStateMonthSalary.yearAndMonth) {
        const now = dayjs();
        this.formStateMonthSalary.yearAndMonth =
          now.date() <= 15 ? now.subtract(1, 'month').format('YYYY-MM') : now.format('YYYY-MM');
      }
    ...
    async queryOutsourceMonthSalaryOffer() {
      ...
      if (!this.formStateMonthSalaryOffer.yearAndMonth) {
        const now = dayjs();
        this.formStateMonthSalaryOffer.yearAndMonth =
          now.date() <= 15 ? now.subtract(1, 'month').format('YYYY-MM') : now.format('YYYY-MM');
      }
    ...
    handleAddPersonContract() {
      this.outsourceContractForm = {
        ...
        contractType:
          this.outsourceContractForm.contractType ||
          (this.outsourcePersonDetail.jobType == '全职' ? '全职合同' : '兼职合同'),
        contractCycle:
          this.outsourceContractForm.contractCycle ||
          (this.outsourcePersonDetail.jobType == '全职' ? '' : '长期'),
      ...
      this.outsourceBasicForm = {
        ...
        currentStatus:
          this.outsourcePersonDetail.currentStatus === '1'
            ? '2'
            : this.outsourcePersonDetail.currentStatus,
      ...
      if (this.outsourcePersonDetail.jobType == '兼职') {
        ...
        this.outsourceSocialSecurityForm = {
          ...
          shebaoYujiaoTime:
            ...
            this.outsourcePersonDetail.planEntryTime
              ? formatToDate(this.outsourcePersonDetail.planEntryTime)
              : '',
          shebaoYutingTime:
            this.outsourceSocialSecurityForm.shebaoYutingTime ||
            this.outsourcePersonDetail.planLeaveTime
              ? formatToDate(this.outsourcePersonDetail.planLeaveTime)
              : '',
          yijinYujiaoTime:
            this.outsourceSocialSecurityForm.yijinYujiaoTime ||
            this.outsourcePersonDetail.planEntryTime
              ? formatToDate(this.outsourcePersonDetail.planEntryTime)
              : '',
          yijinYutingTime:
            this.outsourceSocialSecurityForm.yijinYutingTime ||
            this.outsourcePersonDetail.planLeaveTime
              ? formatToDate(this.outsourcePersonDetail.planLeaveTime)
              : '',
        };
    ...
    async outsourcePersonMoney(outsourcePersonMoney) {
      ...
      const resFormula = await fetchApi.queryOutsourceFormula(formData);
      if (resFormula.code == 1) {
        this.outsourceFormulaList = resFormula.info?.list || [];
      }
    ...
    async updateOutsourceDetail(costOfferDetailsForm: OfferDetailsItem[]) {
      const res = await fetchApi.updateOutsourceDetail(costOfferDetailsForm);
      return res;
    },
    //同步社保
    async updateOutsourceSheBaoMonth(personId: string, jinxinMonth: string) {
      const formData = new FormData();
      formData.append('personId', personId || '');
      formData.append('jinxinMonth', jinxinMonth || '');
      const res = await fetchApi.updateOutsourceSheBaoMonth(formData);
      return res;
    },
    async handleOutsourceMonthSalaryOfferAllocation(record) {
      ...
      formData.append('bId', record.bId || '');
      formData.append('city', record.city || '');
      formData.append('jobType', record.jobType || '');
      formData.append('mId', record.mId || '');
      const gongShiByCompany = await fetchApi.queryOutsourceGongShiByCompany(formData);
      if (gongShiByCompany.code == 1) {
        this.outsourcePersonPerformanceDetailGongShi =
          gongShiByCompany.info as OutsourceFormulaItem[];
      }
      ...
      if (resPerson.code == 1) {
        ...
        this.haveZhaoFlag = resPerson.info[0].haveZhao || '';
        formData.append('phoneNumber', resPerson.info[0].phoneNumber || '');
        formData.append('rId', resPerson.info[0].rId || '');
        let jinxinMonthTemp = record.jinxinMonth + '-01';
        if (resPerson.info[0].realEntryTime) {
          const entryTime = formatToDate(resPerson.info[0].realEntryTime);
          if (dayjs(jinxinMonthTemp).isBefore(entryTime)) {
            jinxinMonthTemp = entryTime;
          }
        }
        formData.append('jinxinStart', jinxinMonthTemp || '');
        let jinxinMonthTempEnd = dayjs(record.jinxinMonth + '-01')
          .endOf('month')
          .format('YYYY-MM-DD');
        if (resPerson.info[0].realLeaveTime) {
          const realLeaveTime = formatToDate(resPerson.info[0].realLeaveTime);
          if (dayjs(realLeaveTime).isBefore(jinxinMonthTempEnd)) {
            jinxinMonthTemp = realLeaveTime;
          }
        }
      ...
      this.costOfferDetailsForm = [] as OfferDetailsItem[];
      let tempFlag = false;
        ...
        if (resTeamMsg.code == 1) {
          ...
          if (tempFlag) {
            ...
            if (this.haveZhaoFlag == '有招') {
              //const manageChargeTax = resTeamMsg?.info[0]?.outFlag === '1' ? (parseFloat(record.manageChargeTax) * 0.49).toFixed(2) : (parseFloat(record.manageChargeTax) * 0.46).toFixed(2);
              temp.manageChargeTax = '0';
              temp.manageChargeRate = '0';
              temp.manageChargeAfter = '0';
              ...
              const manageChargeAllocationTax =
                resTeamMsg?.info[0]?.outFlag === '1'
                  ? (parseFloat(record.manageChargeAllocationTax) * 0.49).toFixed(2)
              ...
              temp.manageChargeAfter = (
                parseFloat(manageChargeAllocationTax) / manageChargeRate
              ).toFixed(2);
              temp.zhuanChargeTax = (parseFloat(record.zhuanChargeTax) * 0.25).toFixed(2);
              temp.zhuanChargeRate = '25%';
              temp.zhuanChargeAfter =
      ...
      if (record.recruitId) {
        const formDataTemp = new FormData();
        formDataTemp.append('id', record.recruitId || '');
        const resTeamMsg = await fetchApi.queryOutsourceTeamMsg(formDataTemp);
        if (resTeamMsg.code == 1) {
          let temp = {} as OfferDetailsItem;
          temp.personId = record.personId || '';
          ...
          if (tempFlag) {
            ...
            if (this.haveZhaoFlag == '有招') {
              const manageChargeAllocationTax = (
                parseFloat(record.manageChargeAllocationTax) * 0.3
              ).toFixed(2);
              temp.manageChargeTax = manageChargeAllocationTax;
              ...
              const manageChargeAllocationTax =
                resTeamMsg?.info[0]?.outFlag === '1'
                  ? (parseFloat(record.manageChargeAllocationTax) * 0.17).toFixed(2)
      ...
      if (record.serviceRecruitId) {
        const formDataTemp = new FormData();
        formDataTemp.append('id', record.serviceRecruitId || '');
        const resTeamMsg = await fetchApi.queryOutsourceTeamMsg(formDataTemp);
        if (resTeamMsg.code == 1) {
          let temp = {} as OfferDetailsItem;
          temp.personId = record.personId || '';
          ...
          temp.teamName =
            record.serviceRecruitId != '5485' ? resTeamMsg?.info[0]?.teamName : '公司';
          temp.outFlag = resTeamMsg?.info[0]?.outFlag || '2';
          if (tempFlag) {
            const tempDetail = record.offerOutSourceDetails?.filter(
            ...
            if (this.haveZhaoFlag == '有招') {
              const manageChargeAllocationTax = (
                parseFloat(record.manageChargeAllocationTax) * 0.4
              ).toFixed(2);
              temp.manageChargeTax = manageChargeAllocationTax;
        ...
        if (record.discoverRecruitId) {
          ...
          if (resTeamMsg.code == 1) {
            ...
            if (tempFlag) {
              ...
              if (this.haveZhaoFlag == '有招') {
                const manageChargeAllocationTax = (
                  parseFloat(record.manageChargeAllocationTax) * 0.15
                ).toFixed(2);
                temp.manageChargeTax = manageChargeAllocationTax;
                ...
                temp.zhuanChargeAfter =
                  ((parseFloat(record.zhuanChargeTax) * 0.125) / manageChargeRate).toFixed(2) ||
                  '0';
              } else {
                const manageChargeAllocationTax =
                  resTeamMsg?.info[0]?.outFlag === '1'
                    ? (parseFloat(record.manageChargeAllocationTax) * 0.085).toFixed(2)
     ...
     * @returns
     */
    async addOutsourceCompanyExcel(payload) {
      try {
        const res = await fetchApi.addOutsourceCompanyExcel(payload);
        return res;
      } catch (error) {
        return null;
      }
     ...
     * @returns
     */
    async queryOutsourceCompanyExcel(companyName: string) {
      try {
        const formData = new FormData();
        formData.append('companyName', companyName || '');
     ...
     * @returns
     */
    async queryOutsourceSalaryCommitCollectSheBao(jinXinMonth: string, companyName?: string) {
      try {
        const formdata = new FormData();
        formdata.append('jinXinMonth', jinXinMonth || '');
        formdata.append('companyName', companyName || '');
        const res = await fetchApi.queryOutsourceSalaryCommitCollectSheBao(formdata);
        if (res.code === 1) {
          this.outsourcePersonSalaryCommitCollectSheBao = res.info || [];
        }
        return res;
      } catch (error) {
        return null;
      ...
      try {
        const payload = outsourceMonthSalaryCaigou
          ...
          .map((item) => ({
            ...
            applyRealNameEn: (loginVueUser.loginName || '').replace(/\+|%2B/g, ' '),
          }));
    ...
    async queryOutsourceSalaryPurchase() {
      try {
        const params = new FormData();
        params.append('pageNumber', this.pageOutsourceSalaryPurchaseList.pageNumber.toString());
        params.append('pageSize', this.pageOutsourceSalaryPurchaseList.pageSize.toString());
        const res = await fetchApi.queryOutsourceSalaryPurchase(params);
        if (res.code === 1) {
          this.outsourceSalaryPurchaseList = res.info.list || [];
          this.pageOutsourceSalaryPurchaseList = {
            pageNumber: res.info.pageNumber,
            pageSize: res.info.pageSize,
            total: res.info.totalCount,
          } as PageItem;
